...
int fftCore(cl_mem dst, cl_mem src, cl_mem spin, cl_int m, enum Mode direction)
{
    ...
    switch (direction) {
        case forward: flag = 0x0000000000000000UL; break;
        case inverse: flag = 0x8000000000000000UL; break;
    }
    ...
    ret = clSetKernelArg(bfly, 5, sizeof(cl_ulong), (void *)&flag);
    ...
    for(iter=1; iter<=m; iter++){
        ...
        ret = clEnqueueNDRangeKernel(queue, bfly, 2, NULL, gws, lws, 0, NULL, NULL);
    ...
    if (direction == inverse) {
        setWorkSize(gws, lws, n, n);
        ret = clEnqueueNDRangeKernel(queue, norm, 2, NULL, gws, lws, 0, NULL, NULL);
...
int main()
{
    ...
    xm = (cl_double2 *)malloc(n * n * sizeof(cl_double2));
    rm = (cl_double2 *)malloc(n * n * sizeof(cl_double2));
    wm = (cl_double2 *)malloc(n / 2 * sizeof(cl_double2));

    for( i = 0; i < n; i++ ) {
        for( j = 0; j < n; j++ ) {
            ((double*)xm)[(2*n*j)+2*i+0] = (double)ipgm.buf[n*j+i];
            ((double*)xm)[(2*n*j)+2*i+1] = (double)0;
        }
    }
    ...
    /* バッファオブジェクトの作成 */
    xmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, n*n*sizeof(cl_double2), NULL, &ret);
    rmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, n*n*sizeof(cl_double2), NULL, &ret);
    wmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, (n/2)*sizeof(cl_double2), NULL, &ret);

    /* メモリバッファにデータを転送 */
    ret = clEnqueueWriteBuffer(queue, xmobj, CL_TRUE, 0, n*n*sizeof(cl_double2), xm, 0, NULL, NULL);
    ...
    /* メモリバッファから結果を取得 */
    ret = clEnqueueReadBuffer(queue, xmobj, CL_TRUE, 0, n*n*sizeof(cl_double2), xm, 0, NULL, NULL);

    /* 振幅の計算 */
    double* ampd;
    ampd = (double*)malloc(n*n*sizeof(double));
    for (i=0; i<n; i++) {
        for (j=0; j<n; j++) {
            ampd[n*((i))+((j))] = (AMP(((double*)xm)[(2*n*i)+2*j], ((double*)xm)[(2*n*i)+2*j+1]));
        }
    }
    opgm.width = n;
    opgm.height = n;
    normalizePGM(&opgm, ampd);
    free(ampd);
