...
{
    cl_int num_rand = 4096*256*32;        /* 생성할 난수의 개수 */
    int count_all, i;
    ...
    clSetKernelArg(kernel_mt, 4, sizeof(cl_uint)*17*num_work_item, NULL); /* 로컬 메모리(state)*/
    clSetKernelArg(kernel_mt, 5, sizeof(mt_struct)*num_work_item, NULL); /* 로컬 메모리(mts) */
